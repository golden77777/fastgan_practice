...
def train(args):
    ...
    optimizerG = optim.Adam(net_decoder.parameters(), lr=nlr, betas=(nbeta1, 0.999))
        ...
        if iteration % (save_interval*10) == 0:
            ...
            with torch.no_grad():
                vutils.save_image( torch.cat([
                        target_image, g_imag]).add(1).mul(0.5), saved_image_folder+'/rec_%d.jpg'%iteration )

        if iteration % (save_interval*50) == 0 or iteration == total_iterations:
            torch.save({'d':netD.state_dict(),'dec':net_decoder.state_dict()}, saved_model_folder+'/%d.pth'%iteration)
